...
- **8-bit storage.** Layer pixels are Canvas 2D `ImageData` (8-bit per channel); the 16-bit path exists only inside the adjustment pipeline (emulated working spaces), and the GPU path only for the tone-LUT stage. Full 16/32-bit storage and compositing remain future tracks.
