...
- **Panels (Window menu):** Color · Adjustments · Properties · Layers · Paths · History · Actions · Navigator · Channels · Metadata — in **two docks**: drag a panel's header into the other dock (a dashed drop zone appears at each dock's tail; the left dock collapses away while unused), or **float any panel** over the canvas with its header pin — floating panels move by their grip bar, stack (last touched on top), collapse, and dock back with the pin. **Window ▸ Workspaces…** saves named layouts (visibility, dock membership + order, floats, collapsed states) to apply in one click; **Reset workspace** restores the factory layout including the docks.
