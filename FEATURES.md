...
- **Panels (Window menu):** Color · Swatches · Adjustments · Properties · Layers · Paths · History · Actions · Navigator · Channels · Metadata — in **two docks**: drag a panel's header into the other dock (a dashed drop zone appears at each dock's tail; the left dock collapses away while unused), or **float any panel** over the canvas with its header pin — floating panels move by their grip bar, stack (last touched on top), collapse, and dock back with the pin. **Window ▸ Workspaces…** saves named layouts (visibility, dock membership + order, floats, collapsed states) to apply in one click; **Reset workspace** restores the factory layout including the docks.
- **Channels panel:** a live histogram of the composite — R/G/B curves (additive overlaps) plus a **Luminosity** curve, each toggleable; **clipping readouts** under the graph show the share of pixels at pure black/white across the enabled channels (warn-tinted above 0.5%); with an active selection the whole histogram **scopes to the selected pixels** (rotations honoured, "Selection" badge on the graph); and the active layer's **mask** appears as a fourth row with its own tonal curve and an eye that views the mask on canvas.
