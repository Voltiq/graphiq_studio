...
- The LRU byte budget is configurable in **Preferences ▸ Performance** (64–1024 MB slider) with live statistics — memory used vs budget, cached-product count, and the session hit rate.
...
- Remaining ceiling: *effect* and *smart-filter* recomputation still processes at full document size on a miss — their kernels/offsets spread pixels, so region-scoping them needs padded-region math (deliberately not attempted; correctness first).
