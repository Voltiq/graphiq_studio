...
export function serializeTextEditor(
  ...
  const walk = (node: Node, deco: { u: boolean; s: boolean }) => {
    if (node.nodeType === Node.TEXT_NODE) {
      const parent = node.parentElement;
      let st: TextRunStyle = { ...base, underline: deco.u, strike: deco.s };
      if (parent) {
    ...
    if (node.tagName === "BR") {
      push("\n", { ...base, underline: false, strike: false });
      return;
    }
    ...
    const childDeco = {
      u: deco.u || line.includes("underline") || node.tagName === "U",
      s: deco.s || line.includes("line-through") || node.tagName === "S" || node.tagName === "STRIKE",
    };
    const isBlock = cs.display === "block" || cs.display === "list-item";
    if (isBlock && text && !text.endsWith("\n")) push("\n", { ...base, underline: false, strike: false });
    for (const child of Array.from(node.childNodes)) walk(child, childDeco);
  };
  for (const child of Array.from(el.childNodes)) walk(child, { u: false, s: false });
...
export function applyPatchToSelection(el: HTMLElement, patch: TextStylePatch): boolean {
  ...
  if (patch.fontSize !== undefined) {
    // execCommand only knows the legacy 1–7 sizes — wrap the range ourselves.
    const span = document.createElement("span");
