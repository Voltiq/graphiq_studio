    ...
    sections: [
      {
        ...
        entries: [
          ...
          { title: "Crop", body: "Ratio presets or free, rule-of-thirds/grid/golden overlays, a darkening shield, and a straighten angle that levels the image as it crops. Straightening rotates the picture, which normally leaves the corners empty — turn on Content-aware and they are filled from the surrounding image instead, so you keep the whole frame. Perspective mode turns the box into a free quad you drag onto a skewed subject, un-skewing it on apply.", keys: "C" },
      ...
      },
      {
        ...
        entries: [
          ...
          { title: "Erase to History", body: "Tick this in the eraser's options and the eraser paints pixels BACK from the history source instead of removing them — pick the state or snapshot to return from in the History panel, then brush over what you want restored. Ideal for taking back part of an edit: blur a whole layer, then erase-to-history over the parts that should stay sharp. On a layer mask, a filter mask or in Quick Mask it's ignored, since a mask has no earlier state to paint back from." },
    ...
    sections: [
      {
        ...
        entries: [
          ...
          { title: "Save & load selections", body: "Select ▸ Save selection… keeps the current selection as a named channel that travels with the document; Load selection… brings it back, on its own or combined with what's selected (add, subtract, intersect). Saved selections also live in the Channels panel with a thumbnail — click to select, Ctrl to add, Alt to subtract, Ctrl+Alt to intersect, double-click to rename. They're stored in the .gproj file." },
